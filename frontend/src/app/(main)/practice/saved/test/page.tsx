...
export default function SavedQuestionsTestPage() {
    ...
    const { data, isLoading } = useSavedQuestions({
        page: 1,
        limit: 1000,
    });
    ...
    useEffect(() => {
        const checkLimit = async () => {
            try {
                const result = await testsApi.checkLimit();
                if (!result.allowed) {
    ...
    useEffect(() => {
        if (!checkingLimit && !showLimitModal && data?.questions?.length) {
            startTest(data.questions);
        }
    }, [checkingLimit, data, showLimitModal, startTest]);
    ...
    if (showLimitModal) {
        return (
            <LimitModal
                isOpen={true}
                onClose={() => router.push("/practice/saved")}
            />
        );
    }
        ...
        return (
            <div className="max-w-3xl mx-auto px-4 py-16 text-center">
                ...
                <h1 className="text-2xl font-bold text-gray-900 mb-3">
                    Немає збережених питань для тесту
                </h1>
                <p className="text-gray-600 mb-6">
                    Спочатку збережи кілька питань, а тоді тут можна буде
                    пройти окремий тест тільки по них.
                </p>
                <Button onClick={handleGoBack}>Назад до збережених</Button>
            </div>
        );
    }
