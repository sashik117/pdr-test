...
import { useTicket, useSubmitTest, useTestStore } from "@/hooks";
...
export default function TicketTestPage() {
    ...
    const { data: ticketData, isLoading } = useTicket(
        ticketId,
        !isNaN(ticketId) && questions.length === 0,
    );
    ...
    useEffect(() => {
        if (ticketData?.questions && !checkingLimit && !showLimitModal) {
            startTest(ticketData.questions);
        }
    }, [ticketData, startTest, checkingLimit, showLimitModal]);
    ...
    const handleRetry = () => {
        ...
        setTestResult(null);
        resetTest();
        router.push(`/test/ticket/${ticketId}`);
        router.refresh();
    };
